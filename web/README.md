...
> **可选组件** - MedFusion 核心库的可视化界面，提供工作流编辑、训练监控和模型管理功能。

医学深度学习框架的 Web 界面。**注意**：这是一个可选组件，MedFusion 核心库可以独立使用（通过 CLI 或 Python API）。Web UI 主要为不熟悉命令行的用户提供图形化操作界面。

## 📌 使用场景

- ✅ **适合使用 Web UI**：需要可视化工作流编辑、实时监控训练过程、团队协作
- ✅ **适合使用 CLI**：自动化脚本、批量实验、远程服务器训练、CI/CD 集成

如果你只需要训练模型，推荐直接使用核心库的 CLI 工具（更轻量、更快速）。
...
web logs            # 查看日志
web logs -f         # 实时跟踪日志
web start --daemon  # 后台运行
web start --reload  # 开发模式（热重载）
```

📖 详细文档：[web/CLI_GUIDE.md](web/CLI_GUIDE.md)
