...
const PRESET_BASE: RunSpec = {
  taskType: "multimodal_research",
  templateId: "multimodal_research",
...
export function applyLocalProTaskTemplate(
  taskType: LocalProTaskType,
  base?: RunSpec,
): RunSpec {
  const spec = cloneRunSpec(base ?? createRunSpecPreset("quickstart"));
  spec.taskType = taskType;

  if (taskType === "binary_classification") {
    spec.templateId = "binary_basic";
    spec.projectName = "medfusion-binary";
    spec.experimentName = "binary-baseline";
    spec.description = "本地专业版二分类模板。";
    spec.tags = ["local-pro", "binary", "classification"];
    ...
    spec.model.fusion.hiddenDim = spec.model.vision.featureDim + spec.model.tabular.outputDim;
    spec.training.numEpochs = 8;
    spec.training.useProgressiveTraining = false;
  }

  if (taskType === "multimodal_research") {
    spec.templateId = "multimodal_research";
    spec.projectName = "medfusion-multimodal";
    spec.experimentName = "multimodal-research";
    spec.description = "本地专业版多模态研究模板。";
    spec.tags = ["local-pro", "multimodal", "attention"];
    spec.model.numClasses = 2;
    ...
    spec.training.numEpochs = 12;
    spec.training.useProgressiveTraining = true;
    spec.training.stage1Epochs = 4;
    spec.training.stage2Epochs = 5;
    spec.training.stage3Epochs = 3;
  }

  if (taskType === "cox_survival") {
    spec.templateId = "cox_survival";
    spec.projectName = "medfusion-cox";
    spec.experimentName = "cox-survival";
    spec.description = "本地专业版 Cox 生存分析模板（当前执行链待补齐）。";
    spec.tags = ["local-pro", "survival", "cox"];
    spec.model.vision.backbone = "resnet18";
    spec.model.vision.featureDim = 128;
    spec.model.fusion.fusionType = "attention";
    spec.model.fusion.hiddenDim = 128;
    spec.training.numEpochs = 12;
    spec.training.useProgressiveTraining = false;
  }
...
export function validateRunSpec(spec: RunSpec): RunSpecValidationIssue[] {
  ...
  if (spec.taskType === "cox_survival") {
    issues.push({
      level: "error",
      message:
        "当前真实 CLI/Web 训练主链尚未支持 Cox 生存分析执行。此模板目前仅用于项目占位和后续实现规划。",
    });
  }
...
export function buildYamlFromRunSpec(spec: RunSpec): string {
  ...
  const header = [
    ...
    spec.taskType ? `# Task type: ${spec.taskType}` : null,
    spec.templateId ? `# Template: ${spec.templateId}` : null,
    spec.taskType === "cox_survival"
      ? "# WARNING: Cox survival execution is not wired into the current CLI/Web training mainline yet."
      : null,
