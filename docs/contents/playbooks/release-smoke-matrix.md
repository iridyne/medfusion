...
| 独立评估模块可基于现有 checkpoint 生成结果 | 必须通过 | 必须通过 | 必须通过 |
| 结果详情可显示来源链 | 必须通过 | 必须通过 | 必须通过 |
| 用户自定义模型能落到本地文件系统 | 必须通过 | 必须通过 | 方向说明 |
| 用户自定义模型具备本地 git 快照历史 | 必须通过 | 必须通过 | 方向说明 |
...
5. 确认模型数据库链：
   ...
   - `/config/model/custom`
   - 官方模板可带回 Run Wizard
   - 用户自定义模型保存后可在本机用户数据目录中找到对应文件
   - 自定义模型在保存 / 删除 / 恢复后可看到本地 git 历史记录
6. 确认独立评估链：
   - `/evaluation`
   - 基于现有 `config + checkpoint` 可生成 `validation / summary / report`
   - 勾选“导入结果后台”后，可直接跳到 `/models`
