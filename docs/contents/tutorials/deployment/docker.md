...
当前仓库的 Docker 真源文件在 `docker/` 目录下：

- Dockerfile: `docker/Dockerfile`
- Compose: `docker/docker-compose.yml`

下面所有构建与编排命令都应显式使用这两个路径，避免在仓库根目录假设不存在的 `Dockerfile` / `docker-compose.yml`。

### 一键启动（CPU 版本）

```bash
# 构建镜像
docker build -f docker/Dockerfile -t medfusion/medfusion:latest .
...
# 构建并指定版本标签
docker build -f docker/Dockerfile -t medfusion/medfusion:0.3.0 .
```
...
```bash
# 启动开发容器
docker compose -f docker/docker-compose.yml --profile dev up -d medfusion-dev
