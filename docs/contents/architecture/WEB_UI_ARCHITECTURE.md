...
- [x] 添加数据管理命令（`medfusion data info/backup/restore/clean`）

#### 6.1.1 模型数据库与高级模式 contract 路线（当前主线）

当前已经明确的产品判断是：

- **模型数据库是唯一官方真源**
- **高级模式保留，但降级为模型数据库的高级编辑视图**
- **当前先做“方案 1”，再有节制地逼近“方案 2”**

这里的两个方案不是 UI 选择，而是**真源边界**选择：

1. 方案 1  
   模型数据库负责“对象、边界、规则、提示”，高级模式编译器负责“如何真正生成配置”
2. 方案 2  
   模型数据库连“如何生成配置”也一起承载，高级模式编译器更像一个解释器

当前正式执行路线：

**A. 已完成：对象真源统一**

- 官方最小组织单元和官方成品模型已经集中到模型数据库
- 高级模式的组件与蓝图来源已经从模型数据库投影
- 高级模式不再维护第二套面向用户的官方目录

**B. 已完成：结构 contract 开始统一**

- 模型数据库现在已经开始承载高级模式结构 contract：
  - family projection
  - family labels
  - status labels
  - required families
...
但**不会立刻把完整 `RunSpec patch` 全量元数据化**。

**D. 暂不进入：完整编译行为 contract 化**

当前明确后置、不作为这一轮路线的内容：

- 把所有 component -> RunSpec patch 全部搬进元数据
- 做一个通用规则引擎式编译器
- 把高级模式编译器完全降成“纯解释器”
...
- patch 合并顺序会迅速复杂化
- 条件覆盖关系会变难排错
- contract 会膨胀，后续演进成本反而上升

正式版当前更稳的行业惯例是：

- **声明式 contract** 承载结构边界和规则
- **薄编译器代码** 承载真正的配置生成和合并逻辑

这也是当前推荐执行路线的核心。

#### 6.1.2 执行计划（针对“先做 1，再逼近 2”）
...
只在下面条件同时成立时，再考虑把更多编译行为 contract 化：

- 模型数据库 contract 已稳定 2 个迭代以上
- 高级模式与模型数据库不再发生规则漂移
- 自定义模型和官方模型的边界仍然清楚
- 现有 Python 编译器的 patch 逻辑已经足够稳定，适合抽象

到那时，才考虑**选择性**把少量稳定 patch 规则元数据化，而不是一次性全部搬过去。
