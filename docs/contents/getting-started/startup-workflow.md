...
- 官方模型真源集中在模型数据库；高级模式只作为官方模型数据库的高级编辑视图。
- 用户自定义模型当前默认保留为本机用户数据目录下的私有文件来源。
- 自定义模型文件当前带 `schema_version`，并通过本地 git 快照保留版本记录；但仍先按内部状态文件处理，不作为用户导入/导出的正式资产格式。
...
3. `/config/model`：查看官方模型数据库；需要时再从这里进入 `/config/model/custom` 组合本地自定义模型
3. `/training`：启动并观察训练
4. `/models`：查看结果、导入 run、交付复盘，并可基于当前结果重开配置
5. `/evaluation`：当你已经有 `config + checkpoint`，但不想重跑训练时，可单独补跑评估与结果构建，并可选导入 `/models`
6. 从结果后台重开配置进入训练时，训练页会标记来源为结果驱动迭代链路
7. 若只做快速验证，可在结果页直接重跑训练（同样保留来源标记）
8. 在训练看板也可基于当前任务重开配置，再次进入训练时会标记为训练阶段驱动迭代链路
9. 跨页重开配置会尽量回填关键模型字段（`backbone`、`numClasses`），减少重复输入
...
- 如果只是想选一个官方模型或组合一个本地自定义模型，优先使用模型数据库而不是直接进入高级模式
- 如果训练已经完成、只是要补跑评估或重新构建结果，优先使用 `/evaluation`，不要为了结果产物再重启一次训练
