...
- 这页是教学型教程，用来帮助你理解配置结构和训练流程
- 它**不是当前最推荐的新手起点**
- 如果你现在的目标是“给自己的任务新建一份 YAML”，优先做法不是从零手写，而是先看 [如何新建模型与 YAML](model-creation-paths.md)

当前更推荐的新手路径是：

1. 先看 [CLI 与 Config 使用路径](cli-config-workflow.md)
2. 再复制主链模板
3. 跑通 `validate-config -> train -> build-results`
4. 最后再回来看这页理解字段和代码

后文里的 `generate_data.py`、`demo_config.yaml` 和训练脚本都属于教学示意。
它们的目标是解释原理，不是替代当前官方主链模板。
