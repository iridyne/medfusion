...
def _assert_contract_meta(
    ...
    *,
    ...
    _assert_path_equal(meta["source_config_path"], config_path)
    _assert_path_equal(meta["checkpoint_path"], checkpoint_path)
...
def _assert_report_contract(
    ...
    *,
    ...
    metrics_path = (output_dir / "metrics" / "metrics.json").as_posix()
    validation_path = (output_dir / "metrics" / "validation.json").as_posix()
    predictions_path = (output_dir / "metrics" / "predictions.json").as_posix()
    summary_path = (output_dir / "reports" / "summary.json").as_posix()
    report_path = (output_dir / "reports" / "report.md").as_posix()
    ...
    assert f"- metrics.json: {metrics_path}" in report_text
    assert f"- validation.json: {validation_path}" in report_text
    assert f"- predictions.json: {predictions_path}" in report_text
    assert f"- summary.json: {summary_path}" in report_text
    assert f"- report.md: {report_path}" in report_text
    ...
    assert (
        "![ROC 曲线（区分能力）](../artifacts/visualizations/roc_curve.png)"
        in report_text
    )
    assert "![训练曲线](../artifacts/visualizations/training_curves.png)" in report_text
    assert (
        "![混淆矩阵（阳性/阴性判别情况）](../artifacts/visualizations/confusion_matrix.png)"
        in report_text
    )
...
def test_build_results_artifacts_from_real_checkpoint(tmp_path: Path) -> None:
    ...
    _assert_path_equal(
        result.artifact_paths["prediction_path"],
        output_dir / "metrics" / "predictions.json",
    )
    _assert_path_equal(
        result.artifact_paths["predictions_path"],
        output_dir / "metrics" / "predictions.json",
    )
    ...
    _assert_path_equal(
        summary_data["artifacts"]["metrics_path"],
        output_dir / "metrics" / "metrics.json",
    )
    _assert_path_equal(
        summary_data["artifacts"]["validation_path"],
        output_dir / "metrics" / "validation.json",
    )
    _assert_path_equal(
        summary_data["artifacts"]["prediction_path"],
        output_dir / "metrics" / "predictions.json",
    )
    _assert_path_equal(
        summary_data["artifacts"]["predictions_path"],
        output_dir / "metrics" / "predictions.json",
    )
    _assert_path_equal(
        summary_data["artifacts"]["summary_path"],
        output_dir / "reports" / "summary.json",
    )
    _assert_path_equal(
        summary_data["artifacts"]["report_path"],
        output_dir / "reports" / "report.md",
    )
