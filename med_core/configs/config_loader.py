    ...
    if config_class == ExperimentConfig:
        if "data" in data:
            if not isinstance(data["data"], dict):
                raise TypeError(f"Expected dict for 'data', got {type(data['data']).__name__}")
            data["data"] = DataConfig(**data["data"])

        if "model" in data:
            if not isinstance(data["model"], dict):
                raise TypeError(f"Expected dict for 'model', got {type(data['model']).__name__}")
            model_data = data["model"]

            if "vision" in model_data:
                if not isinstance(model_data["vision"], dict):
                    raise TypeError(f"Expected dict for 'model.vision', got {type(model_data['vision']).__name__}")
                model_data["vision"] = VisionConfig(**model_data["vision"])

            if "tabular" in model_data:
                if not isinstance(model_data["tabular"], dict):
                    raise TypeError(f"Expected dict for 'model.tabular', got {type(model_data['tabular']).__name__}")
                model_data["tabular"] = TabularConfig(**model_data["tabular"])

            if "fusion" in model_data:
                if not isinstance(model_data["fusion"], dict):
                    raise TypeError(f"Expected dict for 'model.fusion', got {type(model_data['fusion']).__name__}")
        ...
        if "training" in data:
            if not isinstance(data["training"], dict):
                raise TypeError(f"Expected dict for 'training', got {type(data['training']).__name__}")
            training_data = data["training"]

            if "optimizer" in training_data:
                if not isinstance(training_data["optimizer"], dict):
                    raise TypeError(f"Expected dict for 'training.optimizer', got {type(training_data['optimizer']).__name__}")
                training_data["optimizer"] = OptimizerConfig(**training_data["optimizer"])

            if "scheduler" in training_data:
                if not isinstance(training_data["scheduler"], dict):
                    raise TypeError(f"Expected dict for 'training.scheduler', got {type(training_data['scheduler']).__name__}")
                training_data["scheduler"] = SchedulerConfig(**training_data["scheduler"])

            data["training"] = TrainingConfig(**training_data)

        if "logging" in data:
            if not isinstance(data["logging"], dict):
                raise TypeError(f"Expected dict for 'logging', got {type(data['logging']).__name__}")
