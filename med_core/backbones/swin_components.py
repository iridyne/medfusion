...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim: int, norm_layer: type = nn.LayerNorm, spatial_dims: int = 3):
        super().__init__()
        self.dim = dim
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(
        ...
        in_chans: int = 1,
        embed_dim: int = 48,
        window_size: list[list[int]] = [[4, 4, 4], [4, 4, 4]],
        ...
        **kwargs,
        ...
        ) if spatial_dims == 3 else nn.Conv2d(
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                norm_layer=norm_layer,
                downsample=PatchMerging(int(embed_dim * 2**i_layer), norm_layer, spatial_dims)
...
class SwinTransformerBlock2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Merge windows
        shifted_x = window_reverse(attn_windows, self.window_size, (B, Hp, Wp))  # [B, Hp, Wp, C]
