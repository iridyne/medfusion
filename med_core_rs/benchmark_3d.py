...
for batch_size in [1, 2, 4, 8, 16, 32]:
    ...
    rust_mean, rust_std, _ = benchmark(
        lambda v: med_core_rs.normalize_3d_batch(v, method="minmax"), volumes
    )
    ...
    print(
        f"{batch_size:<10} {rust_mean * 1000:>10.2f}±{rust_std * 1000:.2f}  "
...
for size in [(16, 32, 32), (32, 64, 64), (64, 128, 128), (128, 256, 256)]:
    ...
    # Rust
    rust_mean, rust_std, _ = benchmark(
        lambda v: med_core_rs.normalize_3d_batch(v, method="minmax"), volumes, n_runs=5
    )
    ...
    print(
        f"{size_str:<15} {rust_mean * 1000:>10.2f}±{rust_std * 1000:.2f}  "
        f"{numpy_mean * 1000:>10.2f}±{numpy_std * 1000:.2f}  {speedup:>8.2f}x"
    )
...
# Single volume (Rust)
single_rust_mean, _, _ = benchmark(lambda v: med_core_rs.normalize_3d_minmax(v), volume)

# Batch (Rust)
batch_rust_mean, _, _ = benchmark(
    lambda v: med_core_rs.normalize_3d_batch(v, method="minmax"), volumes_batch
)
...
print(
    f"批量 Rust (16): {batch_rust_mean * 1000:.2f} ms ({batch_rust_mean * 1000 / 16:.2f} ms/体积)"
)
